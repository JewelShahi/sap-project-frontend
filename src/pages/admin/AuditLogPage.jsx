...
import { Search, Activity, FileText, ChevronLeft, ChevronRight, Calendar, FilterX, ShieldCheck, UserPlus, LogIn, Trash2, Edit3 } from "lucide-react";
...
const GLOBAL_GROUPS = [
  ...
  { id: "DELETE", label: "Deletions", icon: <Trash2 size={14} />, values: ["delete document"], color: "bg-error" },
          ...
          <div className="bg-base-200/40 border border-base-300/30 rounded-[2.5rem] p-6 lg:p-10 backdrop-blur-xl shadow-2xl flex flex-col gap-10">
            ...
            <div className="space-y-4">
              <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 flex items-center gap-2">
                <ShieldCheck size={14} className="text-primary"/> Protocol Classification
              </span>
              <div className="flex flex-wrap gap-3 flex-col md:flex-row">
                {GLOBAL_GROUPS.map(opt => (
                  <button
                    ...
                    className={`btn btn-md rounded-2xl font-black uppercase text-[11px] transition-all px-6 group border-0 ${
                      selectedGroups.includes(opt.id) 
